...
(function(){
  ...
  // ---- Estado desvío (LOC-004) ----
  // Un desvío por incidencia (itinerario alternativo) deja el GPS funcionando
  // pero con la posición persistentemente fuera de la ruta de la marcha. Antes
  // ese caso ni contaba como fallo ni vencía el giveup: el seguimiento quedaba
  // esperando para siempre. Comportamiento acordado: aviso DESVÍO + suspender
  // el marcado automático (sin estimadas: el servicio real es otro) y, al
  // volver a ruta, reanudar el seguimiento dejando SIN marca las estaciones
  // cruzadas durante el desvío. El retraso se recalcula solo con la siguiente
  // lectura en ruta (rama de retraso provisional) y la siguiente marca real.
  var DETOUR_CONFIRM = 3;       // lecturas fine fuera de ruta SEGUIDAS para confirmar DESVÍO
  var offRouteCount  = 0;       // contador de esas lecturas consecutivas
  var inDetour       = false;   // DESVÍO confirmado — marcado automático suspendido
  var detourSkipped  = {};      // idx → true: estaciones superadas durante el desvío (quedan sin marca)

  // ---- Estado checkpoint a mitad de tramo (LOC-001) ----
  // Un tren adelantado más de LEAD_MIN cruza la estación antes de que la
  // ventana abra: la marca salía con la hora de apertura (tardía) y el adelanto
  // quedaba capado a ~2 min, error que se heredaba tramo a tramo. Acordado con
  // operación: en tramos largos (≥ CHECKPOINT_SEG_MIN min teóricos), UNA
  // lectura GPS a mitad del recorrido estima la hora real de llegada; si sale
  // adelanto, se amplía el margen de apertura de la ventana (adelanto
  // provisional). Si la lectura falla (túnel) se reintenta 1 vez y después se
  // cae al comportamiento clásico (LEAD_MIN). Coste: 1-2 lecturas por tramo largo.
  var CHECKPOINT_SEG_MIN = 6;   // duración teórica mínima del tramo para tener checkpoint
  var CHECKPOINT_MAX_ADV = 15;  // tope de adelanto provisional aceptado (min)
  var checkpointDoneFor  = -1;  // idx destino cuyo checkpoint ya se ejecutó (o agotó el reintento)
  var checkpointRetryFor = -1;  // idx con un fallo de checkpoint — queda 1 reintento
  var provAdvanceMin     = 0;   // adelanto provisional medido (min, > 0)
  var provAdvanceFor     = -1;  // idx destino al que aplica ese adelanto

  // ---- Suelo de seguimiento (LOC-010: transversales / relevo a mitad de recorrido) ----
  // Un maquinista que se incorpora a mitad de recorrido (p.ej. 6014 en Atocha)
  // no debe seguir desde el origen de la marcha: las estaciones anteriores son
  // de otra tripulación y nunca se marcan aquí. Sin esto, recomputeNext apunta a
  // la 1.ª estación sin marcar (junto al origen, a cientos de km) y el guard LTV
  // congela el seguimiento para siempre. El "suelo" es la 1.ª estación que ESTE
  // maquinista debe seguir. Se fija en 3 niveles: B (hora teórica, provisional al
  // arrancar) → A (1.ª lectura GPS buena, definitivo) → C (marca manual). Entre B
  // y A no se auto-marca nada (un suelo provisional mal puesto no debe marcar).
  var FLOOR_GPS_TIMEOUT_MS = 180000; // 3 min sin fix bueno → promover el suelo de B a definitivo
  var trackFloorIdx       = -1;   // idx del suelo activo (-1 = sin fijar)
  var floorConfirmed      = false;// true cuando A (GPS) o C (manual) fijaron el suelo definitivo
  var floorProvisionalSince = 0;  // ms en que se fijó el suelo provisional de B (para el timeout)
   ...
   */
  // Timestamp fiable de una lectura: la hora del fix que reporta el navegador
  // (p.timestamp, epoch ms — el instante en que el receptor OBTUVO la posición),
  // no la hora en que la promesa resuelve. getCurrentPosition puede tardar varios
  // segundos en entregar el fix; a velocidad de línea (83 m/s) cada segundo de
  // desfase son ~83 m de error en el historial CPA y en la marca (atMs).
  // Guarda de sanidad: si falta o difiere >5 min del reloj del dispositivo
  // (epochs no estándar de navegadores antiguos, reloj GPS desincronizado),
  // se cae a Date.now() — el comportamiento previo a este cambio.
  function fixTimestamp(p){
    var ts = p && p.timestamp;
    if(typeof ts === 'number' && isFinite(ts) &&
       Math.abs(Date.now() - ts) <= 5 * 60 * 1000) return ts;
    return Date.now();
  }
    ...
    watchStart: function(onPos, onErr){
      ...
      return navigator.geolocation.watchPosition(
        function(p){ onPos({
          ...
          ts: fixTimestamp(p)   // misma interfaz que getCurrent (simetría de la capa)
        }); },
  ...
  // HOR-002: sale de la última marca REAL (gps/manual) — una marca 'est' no
  // mide nada, solo repite el delta que había cuando se rellenó; si definiera
  // el delta, un 'est' desviado (túnel largo) arrastraría todas las horas
  // efectivas y ventanas siguientes. Solo si no hay ninguna marca real se
  // recurre a la última 'est' (último recurso: es el mismo delta antiguo).
  function currentDelta(){
    var m = API.getMarch();
    if(!m) return 0;
    var lastReal = -1, lastAny = -1;
    for(var i = 0; i < m.s.length; i++){
      if(m.s[i] && m.s[i].tm != null && API.getMark(i) != null){
        lastAny = i;
        if(API.getMarkSource(i) !== 'est') lastReal = i;  // gps, manual o legado sin fuente
      }
    }
    var lp = (lastReal >= 0) ? lastReal : lastAny;
    if(lp < 0) return 0;
  ...
  // Primera estación seguible aún sin marcar. Las superadas durante un desvío
  // (LOC-004) se saltan: quedan sin marca a propósito y no deben re-vigilarse.
  // LOC-010: nunca por debajo del suelo de seguimiento — las estaciones
  // anteriores al punto de incorporación del maquinista quedan en blanco.
  function recomputeNext(){
    var m = API.getMarch();
    gpsNextIdx = -1;
    if(!m) return;
    var startI = (trackFloorIdx > 1 ? trackFloorIdx : 1);
    for(var i = startI; i < m.s.length; i++){
      if(isMarkable(m, i) && API.getMark(i) == null && !detourSkipped[i]){ gpsNextIdx = i; return; }
    }
  }

  // LOC-010 (nivel B): suelo provisional por hora teórica al arrancar. Devuelve
  // el ÚLTIMO markable cuya hora teórica ya pasó (≤ ahora); si ninguno ha pasado
  // (arranque en origen, antes de la salida) devuelve el primer markable → sin
  // cambios respecto a hoy. Es solo orientación: A (GPS) lo confirmará/corregirá
  // antes de marcar nada. Sin marcas reales aún, effTime == teórica.
  function computeTimeFloor(){
    var m = API.getMarch();
    if(!m) return -1;
    var floor = -1, first = -1;
    for(var i = 1; i < m.s.length; i++){
      if(!isMarkable(m, i)) continue;
      if(first < 0) first = i;
      if(normNow(m.s[i].tm) >= m.s[i].tm) floor = i;  // su hora teórica ya pasó (o es ahora)
    }
    return (floor >= 0) ? floor : first;
  }
  ...
  // ===== Checkpoint a mitad de tramo (LOC-001) ===============================
  // Hora efectiva (min) de la mitad del tramo hacia `idx`, o null si el tramo
  // es corto (lo cubre LEAD_MIN) o no hay fila anterior con hora teórica.
  function checkpointMidEff(idx){
    var m = API.getMarch();
    if(!m || !m.s[idx] || m.s[idx].tm == null) return null;
    var prev = -1;
    for(var i = idx - 1; i >= 0; i--){
      if(m.s[i] && m.s[i].tm != null){ prev = i; break; }
    }
    if(prev < 0) return null;
    var segMin = m.s[idx].tm - m.s[prev].tm;
    if(segMin < CHECKPOINT_SEG_MIN) return null;
    return m.s[prev].tm + currentDelta() + segMin / 2;
  }

  // UNA lectura GPS a mitad de tramo: proyecta sobre la ruta, estima la hora de
  // llegada a la estación (distancia/velocidad) y, si el tren viene adelantado,
  // fija el adelanto provisional que amplía el margen de apertura de la ventana.
  // Comparte cerrojo (pollInFlight/pollSeq) con pollTick: nunca dos lecturas a la vez.
  function runCheckpoint(idx){
    var mySeq = ++pollSeq;
    pollInFlight = true;
    pollInFlightSince = Date.now();
    setStatus('Checkpoint de tramo — comprobando posición hacia ' + stName(idx) + '…');
    GeoSource.getCurrent().then(function(pos){
      if(mySeq !== pollSeq) return;       // resolución zombi superada por el watchdog
      pollInFlight = false;
      if(!tracking || gpsNextIdx !== idx) return;
      var usable = isFineFix(pos) && (pos.accuracy == null || pos.accuracy <= TRACKING_ACCURACY_M);
      var pr = usable ? projectGps(pos.lat, pos.lng) : null;
      if(!usable || !pr){
        checkpointFail(idx, usable ? 'fuera de ruta' : 'lectura no usable');
        return;
      }
      checkpointDoneFor = idx;
      var tc = API.COORDS[stName(idx)];
      var distM = tc ? haversineMeters(pos.lat, pos.lng, tc[0], tc[1]) : null;
      var spd = pos.speed || 0;
      if(distM == null || spd < 5){
        // Sin velocidad fiable (parado en señal, maniobra) no hay ETA que valga;
        // un tren detenido a mitad de tramo no viene adelantado de todos modos.
        logEvent('checkpoint', stName(idx) + ' — sin velocidad fiable; ventana normal');
        return;
      }
      var eff = effTime(idx);
      var etaMin = (distM / spd) / 60;
      ...
      pollInFlight = false;
      if(!tracking || gpsNextIdx !== idx) return;
      checkpointFail(idx, gpsErrorMsg(err));
    });
  }

  // Fallo de checkpoint: 1 reintento (el siguiente tick lo relanza); al segundo
  // fallo se da por agotado y la ventana abre con el margen clásico (LEAD_MIN).
  function checkpointFail(idx, motivo){
    if(checkpointRetryFor !== idx){
      checkpointRetryFor = idx;
      logEvent('checkpoint', stName(idx) + ' — ' + motivo + '; se reintentará', 'cpretry' + idx);
      return;
    }
    checkpointDoneFor = idx;
    logEvent('checkpoint', stName(idx) + ' — ' + motivo + ' (2.º intento); ventana normal (LEAD_MIN)');
  }
  ...
  function pollTick(){
    ...
    if(pollInFlight){
      ...
      pollInFlight = false;
      pollSeq++;   // LOC-008: invalida resolución zombi en el hueco entre watchdog y siguiente lanzamiento
    }
    recomputeNext();
    if(gpsNextIdx < 0){ setStatus('Marcha completada', 'ok'); return; }

    // LOC-010 (respaldo de B): si tras FLOOR_GPS_TIMEOUT_MS el GPS no ha podido
    // confirmar el suelo (túnel al arrancar, sin cobertura), promover el suelo
    // provisional de B a definitivo para no quedar bloqueado sin marcar nunca.
    if(!floorConfirmed && floorProvisionalSince &&
       Date.now() - floorProvisionalSince > FLOOR_GPS_TIMEOUT_MS){
      floorConfirmed = true;
      logEvent('inicio', 'Suelo de seguimiento por hora (sin confirmación GPS en ' +
               Math.round(FLOOR_GPS_TIMEOUT_MS/60000) + ' min) desde ' + stName(trackFloorIdx));
    }
    ...
    if(!windowOpen){
      // LOC-001: el adelanto provisional medido por el checkpoint amplía el
      // margen de apertura — un tren adelantado cruza la estación antes de
      // eff − LEAD_MIN y, sin esto, el GPS seguiría dormido en el paso real.
      var lead = LEAD_MIN + (provAdvanceFor === gpsNextIdx ? provAdvanceMin : 0);
      if(nowM >= eff - lead){ windowOpen = true; gpsFailCount = 0; ltvWait = false; }
      else {
        // LOC-001: en tramos largos, una lectura a mitad de recorrido para
        // detectar adelanto y recentrar la ventana. Solo una vez por tramo
        // (+1 reintento si falla); nunca durante un desvío.
        var midEff = checkpointMidEff(gpsNextIdx);
        if(midEff != null && checkpointDoneFor !== gpsNextIdx && !inDetour &&
           normNow(midEff) >= midEff - 0.5){
          runCheckpoint(gpsNextIdx);
          return;
        }
        var advTxt = (provAdvanceFor === gpsNextIdx && provAdvanceMin >= 1) ?
                     ' · adelanto detectado −' + Math.round(provAdvanceMin) + ' min' : '';
        setStatus('Próxima estación: ' + name + ' · hora prevista ' + fmtHM(eff) + advTxt);
        return;
      }
    }
    ...
    GeoSource.getCurrent().then(function(pos){
      ...
      if(poorReading){
        ...
        if(!isFineFix(pos)) updateStoppedStateCoarse(pos, pos.ts);
        if(isStopped){
          setStatus('Tren detenido (confirmado por antenas) — esperando arranque cerca de ' + name, 'warn');
          return;
        }
        var effImp = effTime(gpsNextIdx);
        // Bloque 1+4 (+LOC-004): si ltvWait, isStopped o inDetour están activos,
        // NO marcar como estimada aunque venza el giveup — el tren está
        // confirmado lejos, sin moverse o fuera de su ruta: un 'est' sería falso.
        if(!ltvWait && !isStopped && !inDetour && normNow(effImp) >= effImp + Math.min(GIVEUP_MIN + gpsFailCount, GIVEUP_MAX)){
      ...
      if(!pr){
        // LOC-004: fuera de ruta con GPS sano. Una lectura suelta puede ser una
        // coordenada imprecisa; N seguidas son un desvío real (fuerza mayor).
        offRouteCount++;
        if(!inDetour && offRouteCount >= DETOUR_CONFIRM){
          inDetour = true;
          // El historial CPA y la última lectura fine apuntaban a la ruta
          // abandonada — invalidarlos para que la vuelta empiece limpia.
          cpaResetHistory(-1);
          lastFineReading = null;
          logEvent('desvio', 'DESVÍO confirmado (' + offRouteCount + ' lecturas fuera de ruta) — marcado automático suspendido');
        }
        if(inDetour){
          setStatus('⚠ DESVÍO — fuera de la ruta · marcado automático suspendido', 'warn');
        } else {
          logEvent('fuera_ruta', 'GPS fuera de la ruta', 'fuera');
          setStatus('GPS fuera de la ruta — ¿tren correcto?', 'warn');
        }
        return;
      }
      offRouteCount = 0;
      // LOC-010 (nivel A): 1.ª lectura GPS buena tras arrancar → fijar el suelo
      // DEFINITIVO en la posición real del tren. Para un relevo a mitad de
      // recorrido, coloca el seguimiento donde está el maquinista (no en el
      // origen, a cientos de km) y desactiva en la práctica el guard LTV porque
      // el objetivo pasa a estar cerca. Hasta este momento no se ha marcado nada
      // (B solo orientaba), así que ningún suelo provisional pudo crear marcas.
      if(!floorConfirmed){
        var prevTarget = gpsNextIdx;
        if(pr.passedOrigIdx != null) trackFloorIdx = pr.passedOrigIdx;
        floorConfirmed = true;
        recomputeNext();
        // Solo re-objetivar (descartar esta lectura) si el suelo CAMBIÓ el
        // objetivo —caso relevo a mitad de recorrido—. En un servicio normal
        // desde origen el objetivo no cambia: se sigue procesando esta misma
        // lectura con normalidad (sin perder un sondeo al arrancar).
        if(gpsNextIdx !== prevTarget){
          logEvent('inicio', 'Suelo de seguimiento fijado por GPS en ' +
                   (gpsNextIdx >= 0 ? stName(gpsNextIdx) : 'idx ' + trackFloorIdx) +
                   ' (tren situado)');
          return;   // re-objetivo limpio en el siguiente tick, ya con el target correcto
        }
      }
      if(inDetour){
        // Fin del desvío: posición proyecta de nuevo sobre la ruta. Las
        // estaciones superadas mientras tanto quedan SIN marca (no se inventan
        // pasos que no se midieron); se apartan del seguimiento y se continúa
        // con la primera que queda por delante. El retraso acumulado lo
        // recoge la rama de retraso provisional en el siguiente sondeo y la
        // siguiente marca real lo hace oficial.
        inDetour = false;
        if(pr.passedOrigIdx != null){
          var mD = API.getMarch();
          for(var di = gpsNextIdx; di >= 0 && di <= pr.passedOrigIdx && di < mD.s.length; di++){
            if(isMarkable(mD, di) && API.getMark(di) == null) detourSkipped[di] = true;
          }
        }
        windowOpen = false;     // la ventana pertenecía a la estación pre-desvío
        recomputeNext();
        logEvent('desvio_fin', 'Fin de desvío — de vuelta en ruta; seguimiento reanudado' +
                 (gpsNextIdx >= 0 ? ' hacia ' + stName(gpsNextIdx) : ''));
        setStatus('Fin de desvío — en ruta de nuevo' +
                  (gpsNextIdx >= 0 ? ' · próxima: ' + stName(gpsNextIdx) : ''), 'ok');
        return;   // este sondeo solo reengancha; el siguiente sigue el flujo normal
      }
      ...
      // Hora real del fix (LOC-002): no Date.now(). La promesa puede resolver
      // segundos después de que el receptor obtuviera la posición; fechar la
      // lectura con su timestamp real corrige ese desfase en el historial CPA
      // (atMs de la marca), en lastFineReading y en el detector de parado.
      var nowMs = pos.ts;
      ...
      // Bloque 1+4 (+LOC-004): suprimido durante ltvWait, isStopped o inDetour —
      // no marcar 'est' si el tren está confirmado lejos, sin moverse o desviado
      // (un túnel durante el desvío llega por esta rama de error).
      var giveUp = !ltvWait && !isStopped && !inDetour && (normNow(eff2) >= eff2 + giveUpMin);
      if(giveUp){
        estimateMark(gpsNextIdx);
      } else {
  ...
  function catchUp(){
    ...
    while(gpsNextIdx >= 0 && guard < 100){
      var eff = effTime(gpsNextIdx);
      // Bloque 1+4 (+LOC-004): si ltvWait, isStopped o inDetour persisten tras
      // background, NO marcar 'est' en chain — el tren está físicamente lejos,
      // parado o desviado. El próximo pollTick reevaluará con dato real.
      if(!ltvWait && !isStopped && !inDetour && normNow(eff) >= eff + GIVEUP_MAX){
  ...
  function startTracking(){
    ...
    // LOC-010 (nivel B): suelo provisional por hora teórica. Orienta de inmediato
    // sin depender del GPS; A lo confirmará/corregirá en la 1.ª lectura buena y
    // hasta entonces no se marca nada (guarda en pollTick). Resuelve el caso del
    // relevo a mitad de recorrido (transversal): no se queda apuntando al origen.
    trackFloorIdx = computeTimeFloor();
    floorConfirmed = false;
    floorProvisionalSince = Date.now();
    recomputeNext();
  ...
  function stopTracking(){
    ...
    slowReadings = [];
    coarseStill  = [];
    inDetour = false;        // LOC-004
    offRouteCount = 0;
    detourSkipped = {};
    checkpointDoneFor = -1;  // LOC-001
    checkpointRetryFor = -1;
    provAdvanceMin = 0;
    provAdvanceFor = -1;
    trackFloorIdx = -1;      // LOC-010
    floorConfirmed = false;
    floorProvisionalSince = 0;
  ...
  API.onMarchaChange(function(){
    ...
    slowReadings = [];
    coarseStill  = [];
    inDetour = false;        // LOC-004: el desvío pertenecía a la marcha anterior
    offRouteCount = 0;
    detourSkipped = {};
    checkpointDoneFor = -1;  // LOC-001: el checkpoint pertenecía a la marcha anterior
    checkpointRetryFor = -1;
    provAdvanceMin = 0;
    provAdvanceFor = -1;
    trackFloorIdx = -1;      // LOC-010: el suelo pertenecía a la marcha anterior
    floorConfirmed = false;
    floorProvisionalSince = 0;
  ...
  API.logManualMark = function(idx){
    ...
    // LOC-010 (nivel C): la marca manual fija el suelo de seguimiento en esa
    // estación y lo confirma. Resuelve la queja del relevo a mitad de recorrido
    // ("aunque marque a mano no coge la ubicación"): ahora marcar a mano sitúa
    // el seguimiento y lo desbloquea, sin botones nuevos. Reusa el botón "marcar".
    if(!floorConfirmed || idx > trackFloorIdx){
      trackFloorIdx = idx;
      floorConfirmed = true;
    }
    windowOpen = false;
  ...
  // LOC-011: exponer startTracking para que index.html pueda reanudar el GPS
  // cuando el maquinista pulsa "Continuar servicio" tras un cierre total del navegador.
  API.startTracking = startTracking;

  buildUI();

  // LOC-011: si el centinela de sesión existe al cargar el módulo, el tracking
  // estaba activo antes de la recarga (actualización PWA, F5 accidental).
  // Rearrancar el GPS sin esperar al maquinista — el chip empieza a buscar
  // satélites desde el primer instante, no cuando el maquinista lo note.
  // 800 ms dejan que index.html termine renderRows y construya march; si no
  // hay marcha, startTracking aborta limpiamente ("No hay marcha seleccionada").
  try {
    if(sessionStorage.getItem('ebula_servicio_activo')) {
